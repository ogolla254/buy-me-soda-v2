...
function initializeMainPage() {
  ...
  if (user) { const username = document.getElementById('username'), paypalMe = document.getElementById('paypalMe'); if (username) username.textContent = user.username; if (paypalMe) paypalMe.textContent = user.paypalMe; }
...
function initializeSignupPage() {
  ...
  form.addEventListener('submit', async event => {
    ...
    try { await API.register(data); showMessage('Account created successfully!', 'success'); setTimeout(() => { window.location.href = 'login.html'; }, 1200); }
...
function initializeLoginPage() {
  ...
  form.addEventListener('submit', async event => {
    ...
    try { const user = await API.login({ email, password }); UserSession.setUser(user); showMessage(`Welcome back, ${user.name || user.username}!`, 'success'); setTimeout(() => { window.location.href = `/${user.username}`; }, 900); }
...
function initializeQRPage() {
  ...
  if (downloadBtn) downloadBtn.addEventListener('click', () => { const canvas = document.querySelector('#qrcode canvas'); if (!canvas) return; const link = document.createElement('a'); link.download = `${user.username}-soda-qr.png`; link.href = canvas.toDataURL(); link.click(); API.trackEvent('qr_download', { username: user.username }); });
}

let currentCreator = null, selectedSodaCount = 1;
function openSupportModal(sodaCount) {
  selectedSodaCount = sodaCount;
  const modal = document.getElementById('supportModal');
  const amount = document.getElementById('supportAmount');
  const form = document.getElementById('supportForm');
  if (!modal || !amount || !form) return;
  amount.textContent = `You are supporting with $${sodaCount}.`;
  form.reset(); modal.classList.add('open'); modal.setAttribute('aria-hidden', 'false');
  document.getElementById('supporterName')?.focus();
}
function closeSupportModal() {
  const modal = document.getElementById('supportModal'); if (!modal) return;
  modal.classList.remove('open'); modal.setAttribute('aria-hidden', 'true');
}

function initializeCreatorPage() {
  const parts = window.location.pathname.split('/').filter(Boolean), username = parts[parts.length - 1];
  ...
  API.getCreator(username).then(creator => {
    ...
    const name = document.getElementById('creatorName'), bio = document.getElementById('creatorBio'), image = document.getElementById('profileImage');
    if (name) name.textContent = creator.name || creator.username; if (bio) bio.textContent = creator.bio || 'Support me by buying me a soda! 🥤'; if (image && creator.profilePicture) image.src = creator.profilePicture;
    ...
    document.getElementById('supportForm')?.addEventListener('submit', async event => {
      ...
      const button = document.getElementById('supportPay');
      const supporterName = String(document.getElementById('supporterName')?.value || '').trim();
      const supporterEmail = String(document.getElementById('supporterEmail')?.value || '').trim();
      const message = String(document.getElementById('supporterMessage')?.value || '').trim();
      ...
      try {
        const result = await API.createSupport({ username: currentCreator.username, sodaCount: selectedSodaCount, supporterName, supporterEmail, message });
        API.trackEvent('support_started', { username: currentCreator.username, sodaCount: selectedSodaCount });
        closeSupportModal();
        showMessage(`Support reference ${result.reference} created. Opening PayPal...`, 'success');
...
function initializeCreatorDashboard() {
  const username = UserSession.getCurrentUser()?.username; if (!username) return void (window.location.href = 'login.html');
  ...
  API.getCreatorDashboard(username).then(data => {
    ...
    if (elements.name) elements.name.textContent = c.name; if (elements.username) elements.username.textContent = `@${c.username}`; if (elements.profileViews) elements.profileViews.textContent = s.profileViews; if (elements.sodas) elements.sodas.textContent = s.sodas; if (elements.transactions) elements.transactions.textContent = s.confirmedTransactions; if (elements.earnings) elements.earnings.textContent = `$${s.earnings.toFixed(2)}`; if (elements.pending) elements.pending.textContent = `$${s.pendingAmount.toFixed(2)}`; if (elements.messages) elements.messages.textContent = s.unreadMessages; if (elements.paypalMe) elements.paypalMe.textContent = c.paypalMe; if (elements.bio) elements.bio.textContent = c.bio || 'No bio yet.'; if (elements.image && c.profilePicture) elements.image.src = c.profilePicture;
    if (elements.activity) elements.activity.innerHTML = data.activity.slice(0, 15).map(e => `<div class="activity-item"><strong>${e.type.replaceAll('_', ' ')}</strong><span>${new Date(e.createdAt).toLocaleString()}</span></div>`).join('') || '<p>No activity yet.</p>';
    if (elements.transactionList) elements.transactionList.innerHTML = data.transactions.slice(0, 20).map(t => `<div class="activity-item"><strong>$${Number(t.amount).toFixed(2)} · ${t.sodaCount} soda${t.sodaCount === 1 ? '' : 's'}</strong><span>${t.status} · ${new Date(t.createdAt).toLocaleString()}</span></div>`).join('') || '<p>No support activity yet.</p>';
  }).catch(error => showMessage(error.message || 'Could not load dashboard', 'error'));
  document.getElementById('dashboardLogout')?.addEventListener('click', () => { UserSession.logout(); window.location.href = 'login.html'; });
}
